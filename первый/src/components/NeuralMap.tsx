    ...
    const render = () => {
      ...
      shops.forEach((shop, i) => {
        ...
        shops.slice(i + 1).forEach(other => {
          ...
          if (dist < 0.05) { // Связи до 5км
            ...
            gradient.addColorStop(0, `rgba(0, 255, 255, ${0.3 * (shop.activity || 0.5)})`);
            gradient.addColorStop(1, `rgba(0, 255, 255, ${0.3 * (other.activity || 0.5)})`);
      ...
      shops.forEach(shop => {
        ...
        if (pulseOrigin?.id === shop.id) {
          ...
          ctx.strokeStyle = `rgba(0, 255, 255, ${(1 - pulse) * 0.8})`;
          ctx.lineWidth = 3;
          ctx.stroke();
